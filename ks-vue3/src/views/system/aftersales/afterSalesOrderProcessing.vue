      ...
      <el-col :span="24" :xs="24">
        ...
        >
          ...
          <el-form-item label="单据类型" prop="serviceType">
            <el-radio-group class="form-item" v-model="form.serviceType">
...
<script setup name="afterSalesOrderProcessing">
...
import {
  ...
  saveAfterSales,
  delAfterSales,
} from "@/api/aftersales/afterSalesOrderProcessing";
...
function deleteReceipt() {
  getAfterSales(form.value.systematicReceipt).then((response) => {
    const details = response.data.details;
    proxy.$modal
      .confirm(
        "确认要删除系统编号为" + form.value.systematicReceipt + "的售后单据?"
      )
      .then(function () {
      ...
      .then(() => {
        proxy.$modal.msgSuccess(
          "已删除系统编号为" + form.value.systematicReceipt + "的售后单据。"
        );
        reset();
      })
      .catch(() => {});
