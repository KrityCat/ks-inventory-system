      ...
      <el-col :span="24" :xs="24">
        ...
        >
          ...
          <el-form-item label="单据类型" prop="receiptType">
            <el-radio-group v-model="form.receiptType" class="form-item">
              <el-radio-button :value="10" label="10">报溢</el-radio-button>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item
              v-hasPermi="['inventory:reportOverflowProcessing:selectUser']"
              label="经手人"
              prop="userIds"
          >
            <el-select
          ...
          <div v-if="finding">
            <el-divider/>
            <el-form-item
                :disabled="['inventory:reportOverflowProcessing:takeEffect']"
                label="审核结果"
                prop="findingOfAudit"
            >
              <el-select
        ...
        <div>
          ...
          <el-affix :offset="20" class="tc mt16 footer" position="bottom">
            <el-button
                v-hasPermi="['inventory:reportOverflowProcessing:add']"
                color="#BB5500"
                icon="Plus"
                round
                @click="addOrder"
            ...
            >
            <el-button
                v-hasPermi="['inventory:reportOverflowProcessing:detail']"
                color="#00AAAA"
                icon="Plus"
                round
                @click="addItem"
            ...
            >
            <el-button
                v-hasPermi="['inventory:reportOverflowProcessing:save']"
                :disabled="submitBtn"
                icon="position"
                round
                type="primary"
            ...
            >
            <el-button
                v-hasPermi="['inventory:reportOverflowProcessing:takeEffect']"
            ...
            >
            <el-button
                v-hasPermi="['inventory:reportOverflowProcessing:printOut']"
                :disabled="printBtn"
                color="#626aef"
                icon="Printer"
                round
            ...
            >
              <el-button
                  ref="ref22"
                  v-hasPermi="['inventory:reportOverflowProcessing:delete']"
                  :disabled="printBtn"
                  icon="delete"
                  round
                  type="danger"
...
<script name="ReportOverflowProcessing" setup>
import {useRouter} from "vue-router";
import {getUserProfile} from "@/api/system/user";
import {delReceipt, getReceipt, saveInventoryReceipt,} from "@/api/inventory/reportOverflowProcessing";
...
/** 新增按钮操作 */
function addOrder() {
  addProcessingBatch(router, proxy, 'RreportOverflowProcessing')
}
...
function submitAuditingForm(isSubmit) {
  proxy.$refs["orderRef"].validate(async (valid) => {
    ...
    if (isSubmit === 1) {
      proxy.$modal.msgSuccess("报溢单据保存成功");
    } else if (isSubmit === 2) {
      form.value.receiptStatus = 2;
      proxy.$modal.msgSuccess("报溢单据审核成功");
    }
...
function deleteReceipt() {
  deleteReceiptBatch(router, proxy, form, getReceipt, delReceipt, 'ReportOverflowProcessing');
}
