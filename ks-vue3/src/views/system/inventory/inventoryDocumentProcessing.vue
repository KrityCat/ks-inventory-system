      ...
      <el-col :span="24" :xs="24">
        ...
        >
          ...
          <el-form-item label="单据类型" prop="receiptType">
            <el-radio-group v-model="form.receiptType" class="form-item">
              <el-radio-button
                  v-hasPermi="[
                  'inventory:inventoryReceiptProcessing:warehousing',
                ]"
                  :value="5"
                  label="5"
                  @change="outbound"
              ...
              >
              <el-radio-button
                  v-hasPermi="['inventory:inventoryReceiptProcessing:Outbound']"
                  :value="6"
                  label="6"
                  @change="retreat"
              >其他出库
          ...
          <el-form-item
              v-hasPermi="['inventory:inventoryReceiptProcessing:selectUser']"
              label="经手人"
              prop="userIds"
          >
            <el-select
          ...
          <div v-if="finding">
            <el-divider/>
            <el-form-item
                :disabled="['inventory:inventoryReceiptProcessing:takeEffect']"
                label="审核结果"
                prop="findingOfAudit"
            >
              <el-select
        ...
        <div>
              ...
              <el-table-column v-if="lockOutbound" align="center" label="锁单货品" prop="lockedGoods" width="120">
                <template #default="scope">
                  <el-radio-group v-model="scope.row.lockedGoods">
                    <el-radio-button v-hasPermi="['inventory:inventoryReceiptProcessing:lockedGoods']" :value="1"
                                     label="1">是
                    </el-radio-button>
          ...
          <el-affix :offset="20" class="tc mt16 footer" position="bottom">
            <el-button
                v-hasPermi="['inventory:inventoryReceiptProcessing:add']"
                color="#BB5500"
                icon="Plus"
                round
                @click="addOrder"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryReceiptProcessing:detail']"
                color="#00AAAA"
                icon="Plus"
                round
                @click="addItem"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryReceiptProcessing:save']"
                :disabled="submitBtn"
                icon="position"
                round
                type="primary"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryReceiptProcessing:takeEffect']"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryReceiptProcessing:printOut']"
                :disabled="printBtn"
                color="#626aef"
                icon="Printer"
                round
...
<script name="InventoryDocumentProcessing" setup>
import {useRouter} from "vue-router";
import {getUserProfile} from "@/api/system/user";
import {delReceipt, getReceipt, saveInventoryReceipt,} from "@/api/inventory/inventoryDocumentProcessing";
...
import {customerList, supplierList, userList, warehouseList,} from "@/api/common/CommonReceipt";
...
/** 新增按钮操作 */
function addOrder() {
  addProcessingBatch(router, proxy, 'InventoryDocumentProcessing')
}

/** 库存入库 */
function outbound() {
  form.value.systematicReceipt = "QR-" + randomId();
...
/** 库存退库 */
function retreat() {
  form.value.systematicReceipt = "QC-" + randomId();
...
function submitAuditingForm(isSubmit) {
  proxy.$refs["orderRef"].validate(async (valid) => {
    ...
    if (isSubmit === 1) {
      proxy.$modal.msgSuccess("其他库存单据保存成功");
    } else if (isSubmit === 2) {
      form.value.receiptStatus = 2;
      proxy.$modal.msgSuccess("其他库存单据审核成功");
    }
...
function getSummaries(param) {
  ...
  if (form.value.receiptType === 6) {
    getColumns(columns, data, form, sums, [1, 2, 3, 4, 5, 6, 7, 13], [14, 15]);
    return sums;
  } else if (form.value.receiptType === 5) {
...
function deleteReceipt() {
  deleteReceiptBatch(router, proxy, form, getReceipt, delReceipt, 'InventoryDocumentProcessing');
}
