        ...
        >
          ...
          <el-form-item label="单据类型" prop="receiptType">
            <el-radio-group class="form-item" v-model="form.receiptType">
              <el-radio-button label="5">其他入库</el-radio-button>
              <el-radio-button label="6" disabled>其他出库</el-radio-button>
            </el-radio-group>
          </el-form-item>
        ...
        <div>
          ...
          <div class="tc mt16 footer">
            ...
            <el-button
              @click="submitForm"
              type="primary"
              v-hasPermi="['system:inventory:saveStorageOther']"
              >保存</el-button
            >
          </div>
        </div>
...
import { getHeadReceipt, saveStorageOther } from "@/api/inventory/storageOther";
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        ...
        saveStorageOther(req).then((response) => {
          this.$modal.msgSuccess("其他入库单保存成功");
          this.cancel();
        });
      });
    },
