        ...
        >
          ...
          <el-form-item label="单据类型" prop="receiptType">
            <el-radio-group class="form-item" v-model="form.receiptType">
              <el-radio-button label="5" disabled>其他入库</el-radio-button>
              <el-radio-button label="6">其他出库</el-radio-button>
            </el-radio-group>
          </el-form-item>
        ...
        <div>
          ...
          <div class="tc mt16 footer">
            ...
            <el-button
              @click="submitForm"
              type="primary"
              v-hasPermi="['system:inventory:saveOutboundOther']"
              >保存</el-button
            >
          </div>
        </div>
...
import {
  getHeadReceipt,
  saveOutboundOther,
} from "@/api/inventory/outboundOther";
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (
          (this.form.supplierIds === null || this.form.supplierIds === "") &&
          (this.form.customerIds === null || this.form.customerIds === "")
        ) {
          return ElNotification({
            title: "错误",
            message: "供应商和客户不能都为空，需选择供应商或客户!",
            type: "error",
          });
        ...
        saveOutboundOther(req).then((response) => {
          this.$modal.msgSuccess("其他出库单保存成功");
          this.cancel();
        });
      });
    },
