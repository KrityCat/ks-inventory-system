      ...
      <el-col :span="24" :xs="24">
        ...
        >
          ...
          <el-form-item label="单据类型" prop="receiptType">
            <el-radio-group v-model="form.receiptType" class="form-item">
              <el-radio-button :value="7" label="7">库存调拨</el-radio-button>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item
              v-hasPermi="['inventory:inventoryTransferProcessing:selectUser']"
              label="经手人"
              prop="userIds"
          >
            <el-select
          ...
          <div v-if="finding">
            <el-divider/>
            <el-form-item
                :disabled="['inventory:inventoryTransferProcessing:takeEffect']"
                label="审核结果"
                prop="findingOfAudit"
            >
              <el-select
        ...
        <div>
          ...
          <el-affix :offset="20" class="tc mt16 footer" position="bottom">
            <el-button
                v-hasPermi="['inventory:inventoryTransferProcessing:add']"
                color="#BB5500"
                icon="Plus"
                round
                @click="addOrder"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryTransferProcessing:detail']"
                color="#00AAAA"
                icon="Plus"
                round
                @click="addItem"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryTransferProcessing:save']"
                :disabled="submitBtn"
                icon="position"
                round
                type="primary"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryTransferProcessing:takeEffect']"
            ...
            >
            <el-button
                v-hasPermi="['inventory:inventoryTransferProcessing:printOut']"
                :disabled="printBtn"
                color="#626aef"
                icon="Printer"
                round
...
<script name="InventoryTransfer" setup>
import {useRouter} from "vue-router";
import {getUserProfile} from "@/api/system/user";
import {delReceipt, getReceipt, saveInventoryTransfer,} from "@/api/inventory/inventoryDocumentProcessing";
...
/** 新增按钮操作 */
function addOrder() {
  addProcessingBatch(router, proxy, 'InventoryTransfer')
}
...
function submitAuditingForm(isSubmit) {
  proxy.$refs["orderRef"].validate(async (valid) => {
    ...
    if (isSubmit === 1) {
      proxy.$modal.msgSuccess("库存调拨单保存成功");
    } else if (isSubmit === 2) {
      form.value.receiptStatus = 2;
      proxy.$modal.msgSuccess("库存调拨单审核成功");
    }
    const reqs = {...form.value, details};
    await saveInventoryTransfer(reqs);
    cancel();
  });
}
...
function deleteReceipt() {
  deleteReceiptBatch(router, proxy, form, getReceipt, delReceipt, 'InventoryTransfer');
}
...
function selectLockInventoryQty(productId) {
  if (!productId || typeof productId !== 'string') {
    proxy.$modal.msgError('请选择货品');
    return;
  }
  ...
  currentProductId.value = productId;
  warehousingId.value = 0;
  retrievalId.value = form.value.retrievalIds;
  openLockInventoryQuantity.value = true;
}
