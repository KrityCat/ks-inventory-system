        ...
        >
          ...
          <el-form-item label="单据类型" prop="receiptType">
            <el-radio-group class="form-item" v-model="form.receiptType">
              <el-radio-button label="7">库存调拨</el-radio-button>
            </el-radio-group>
          </el-form-item>
        ...
        <div>
          ...
          <div class="tc mt16 footer">
            ...
            <el-button
              @click="submitForm"
              type="primary"
              v-hasPermi="['system:inventory:saveInventoryTransfer']"
              >保存</el-button
            >
          </div>
        </div>
...
} from "@/api/inventory/inventoryTransfer";
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        ...
        saveInventoryTransfer(req).then((response) => {
          this.$modal.msgSuccess("库存调拨单保存成功");
          this.cancel();
        });
      });
    },
