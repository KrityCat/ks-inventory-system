        ...
        >
          ...
          <el-form-item label="单据类型" prop="receiptType">
            <el-radio-group class="form-item" v-model="form.receiptType">
              ...
              <el-radio-button label="2" disabled>采购退库</el-radio-button>
            </el-radio-group>
          </el-form-item>
        ...
        <div>
          ...
          <div class="tc mt16 footer">
            ...
            <el-button
              @click="submitForm"
              type="primary"
              v-hasPermi="['system:purchase:saveWarehouseReceipt']"
              >保存</el-button
            >
          </div>
        </div>
...
import {
  ...
  getPurchaseOrder,
} from "@/api/purchase/WarehouseReceipt";
...
export default {
  ...
  computed: {
    money() {
      return parseFloat(this.form.details.planQuantity);
    },
  },
  created() {
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        ...
        saveWarehouseReceipt(req).then((response) => {
          this.$modal.msgSuccess("采购入库单保存成功");
          this.cancel();
        });
      });
    },
